...
const Home = () => {
  ...
  // Fetch pending wholesalers (admin only)
  useEffect(() => {
    if (user?.role === "admin") {
      const fetchPending = async () => {
        try {
          ...
          toast.error("Failed to load pending wholesaler applications");
        } finally {
      ...
      setLoadingPending(false); // Skip loading if not admin
    }
  }, [user]);
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      toast.error("Failed to approve wholesaler");
    } finally {
          ...
          >
            ...
            <div className="space-y-4">
              {[
                ...
                {
                  text: `${stats.products} product${stats.products !== 1 ? "s" : ""} synced`,
                  time: "1 min ago",
                },
              ...
              {loadingPending ? (
                ...
                <div className="space-y-4">
                  {pendingWholesalers.map((applicant) => (
                    <div
                      key={applicant._id}
                      className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-6 bg-gray-50 rounded-xl border border-gray-200 gap-4"
                    >
                      <div>
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        <button
                          ...
                          className="px-5 py-3 bg-emerald-600 text-white rounded-xl font-medium hover:bg-emerald-700 disabled:opacity-70 transition flex items-center gap-2 min-w-[110px]"
                        >
                          <CheckCircle className="w-5 h-5" />
                          {approving[applicant._id]
                            ? "Approving..."
                            : "Approve"}
                        </button>
                        ...
                        <button
                          ...
                          className="px-5 py-3 bg-red-100 text-red-700 rounded-xl font-medium hover:bg-red-200 transition flex items-center gap-2 min-w-[110px]"
                        >
