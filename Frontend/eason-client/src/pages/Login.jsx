// src/pages/Login.jsx — Cleaned & Premium Version (Feb 2025)
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update Zustand store right away
      zustandLogin(user);

      // Force a quick server check to ensure everything is synced
      await checkAuth();

      // Clean redirect based on role (clean & reliable)
      ...
      if (role === "admin") {
        ...
        // Verified wholesaler → marketplace (can add products)
        // Unverified → pending screen
        navigate(
          user.verified ? "/marketplace" : "/pending-approval",
          { replace: true }
        );
      } else {
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 relative overflow-hidden font-['Satoshi'] flex items-center justify-center px-6">
        ...
        <div className="relative z-10 w-full max-w-md">
          <div className="bg-white/80 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/50 p-10 md:p-12">
            <div className="text-center mb-10">
              <h1 className="text-6xl md:text-7xl font-light tracking-tight text-gray-900">eAson</h1>
